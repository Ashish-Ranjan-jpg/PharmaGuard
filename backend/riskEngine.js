...
/**
 * Gene-specific wildtype allele names
 */
const GENE_WILDTYPE = {
  CYP2D6: "*1",
  CYP2C19: "*1",
...
/**
 * Effect priority for diplotype construction.
 * Higher priority = more clinically significant = should be kept when >2 alleles.
 */
const EFFECT_PRIORITY = {
  "non-functional": 4,
  decreased_transport: 3,
  decreased: 3,
  increased: 2,
  functional: 1,
  normal: 1,
...
/**
 * Sort star alleles numerically (e.g., *2 before *10, not *10 before *2).
 */
function sortStarAlleles(a, b) {
  const numA = parseFloat(a.replace(/^\*/, "").replace(/[A-Za-z].*/, "")) || 0;
  const numB = parseFloat(b.replace(/^\*/, "").replace(/[A-Za-z].*/, "")) || 0;
 ...
 * Uses pharmacogenomics-aware logic:
 *  - Gene-specific wildtype alleles (e.g., SLCO1B1 uses *1a)
 *  - Prioritizes non-functional over functional alleles
 *  - Numeric star allele sorting
 */
function buildDiplotype(variants, gene) {
  const geneVariants = variants.filter((v) => v.gene === gene);
  const wildtype = GENE_WILDTYPE[gene] || "*1";

  if (geneVariants.length === 0) {
    return { diplotype: `${wildtype}/${wildtype}`, method: "default_wildtype" };
  }

  // Collect star alleles with their effect priority from VARIANT_DATABASE
  const alleleEntries = []; // { allele, priority, zygosity }
  for (const v of geneVariants) {
    const variantInfo = getVariantInfo(v.rsid);
    const starAllele = (variantInfo && variantInfo.starAllele) || v.starAllele;
    if (!starAllele) continue;

    const effect = variantInfo ? variantInfo.effect : "unknown";
    const priority = EFFECT_PRIORITY[effect] || 0;
    const normalizedAllele = starAllele.startsWith("*")
      ? starAllele
      : `*${starAllele}`;

    if (v.zygosity === "homozygous_variant") {
      alleleEntries.push({ allele: normalizedAllele, priority });
      alleleEntries.push({ allele: normalizedAllele, priority });
    } else if (v.zygosity === "heterozygous") {
      alleleEntries.push({ allele: normalizedAllele, priority });
    }
    // Skip homozygous_reference — patient doesn't carry the variant
  }

  if (alleleEntries.length === 0) {
    return {
      diplotype: `${wildtype}/${wildtype}`,
      method: "no_star_alleles_found",
    };
  }

  // Sort by priority (descending) then by allele name (numeric)
  alleleEntries.sort((a, b) => {
    if (b.priority !== a.priority) return b.priority - a.priority;
    return sortStarAlleles(a.allele, b.allele);
  });

  // Take the top 2 most clinically significant alleles
  let allele1, allele2;
  if (alleleEntries.length >= 2) {
    allele1 = alleleEntries[0].allele;
    allele2 = alleleEntries[1].allele;
  } else {
    // One variant allele + one wildtype
    allele1 = wildtype;
    allele2 = alleleEntries[0].allele;
  }

  // Sort the pair for consistent display (numeric order)
  const sorted = [allele1, allele2].sort(sortStarAlleles);
  const diplotype = `${sorted[0]}/${sorted[1]}`;
...
function predictRisk(parsedVCF, drugName) {
  ...
  if (!primaryGene) {
    return createUnknownResponse(
      parsedVCF,
      drug,
      `Drug "${drug}" is not in the supported drug database.`,
    );
  }
  ...
  // Build diplotype
  const { diplotype, method } = buildDiplotype(
    parsedVCF.pharmacogenomicVariants,
    primaryGene,
  );
...
function calculateConfidence(geneVariants, method, phenotype) {
  ...
  if (geneVariants.length > 1) score += 0.1;

  // Method-based adjustments
  if (method === "variant_based") score += 0.15;
  if (method === "default_wildtype") score -= 0.1;
...
function getMonitoringRecommendations(drug, phenotype) {
  const monitoring = {
    CODEINE: {
      PM: "Monitor for inadequate pain relief. Consider pain scoring.",
      IM: "Monitor pain control efficacy.",
      URM: "Monitor for respiratory depression, sedation, and signs of opioid toxicity.",
      NM: "Standard monitoring.",
      RM: "Monitor for signs of increased opioid effect.",
    },
    CLOPIDOGREL: {
      PM: "Monitor platelet function. Consider VerifyNow P2Y12 assay.",
      IM: "Monitor platelet reactivity. Consider platelet function testing.",
      NM: "Standard monitoring.",
      RM: "Standard monitoring.",
      URM: "Monitor for bleeding.",
    },
    WARFARIN: {
      PM: "INR monitoring every 1-3 days initially. Watch for bleeding signs.",
      IM: "INR monitoring twice weekly initially. Adjust dose to target INR 2-3.",
      NM: "Standard INR monitoring per protocol.",
    },
    SIMVASTATIN: {
      PM: "Monitor CK levels every 2-4 weeks. Report any muscle pain immediately.",
      IM: "Monitor CK levels. Report muscle symptoms.",
      NM: "Standard lipid panel monitoring.",
    },
    AZATHIOPRINE: {
      PM: "CBC with differential WEEKLY. Monitor for signs of infection.",
      IM: "CBC weekly for first 2-3 months, then monthly.",
      NM: "CBC monthly after initial weekly monitoring.",
    },
    FLUOROURACIL: {
      PM: "INTENSIVE monitoring: CBC, hepatic/renal function. Watch for mucositis, diarrhea, hand-foot syndrome.",
      IM: "Enhanced monitoring of CBC and toxicity signs.",
      NM: "Standard chemotherapy monitoring protocol.",
    },
  };

  return (
    monitoring[drug]?.[phenotype] || "Standard clinical monitoring recommended."
  );
}
...
function getAlternativeDrugs(drug, phenotype) {
  ...
  const alternatives = {
    CODEINE: [
      "Morphine (direct agonist)",
      "Acetaminophen",
      "NSAIDs (ibuprofen)",
      "Tramadol (caution — also CYP2D6)",
    ],
